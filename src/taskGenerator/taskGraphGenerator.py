...
class RandomTaskGenerator:
    ...
    def generate_topology(self, num_tasks, num_cores, edge_probability, edge_weights_lb, edge_weights_ub, source_fraction, sink_fraction, output_path):
                ...
                for i in range(n):
                    for graph_size in self.graph_sizes:
                        for probability in self.probabilities:
                            graphs.append(self.generate_topology(graph_size, num_cores, probability, edge_weight_lbs[wi], edge_weight_ubs[wi], 0.1, 1, output_path + "img/" + "graph_" + str(index) + "_n" + str(graph_size) + "_" + str((wi * num_instances) + instance_id) + ".png"))
                            index += 1
